...
def process_kanji_readings(
    ...
    for level, items in data.items():
        ...
        for item in items:
            front = f'<div style="font-size: 10px; color: #666;">Kanji - Level {level_num}</div><div style="font-size: 24px; color: #FF8C00; text-align: center; margin: 15px 0;">{item["character"]}</div><div style="font-size: 10px; color: #50C878;">Reading</div>'
            ...
            if onyomi_readings:
                tags = "".join(
                    [
                        f'<span style="display: inline-block; padding: 3px 8px; margin: 2px; background: rgba(74, 144, 226, 0.2); color: #2C5AA0; border-radius: 3px; font-size: 8px;">{reading}</span>'
                        for reading in onyomi_readings
                    ]
                )
                reading_lines.append(
                    f'<div style="margin-bottom: 6px;"><span style="font-size: 12px; font-weight: bold; color: #4A90E2;">On\'yomi:</span> {tags}</div>'
                )

            if kunyomi_readings:
                tags = "".join(
                    [
                        f'<span style="display: inline-block; padding: 3px 8px; margin: 2px; background: rgba(80, 200, 120, 0.2); color: #2E7D46; border-radius: 3px; font-size: 8px;">{reading}</span>'
                        for reading in kunyomi_readings
                    ]
                )
                reading_lines.append(
                    f'<div style="margin-bottom: 6px;"><span style="font-size: 12px; font-weight: bold; color: #50C878;">Kun\'yomi:</span> {tags}</div>'
                )

            if nanori_readings:
                tags = "".join(
                    [
                        f'<span style="display: inline-block; padding: 3px 8px; margin: 2px; background: rgba(155, 89, 182, 0.2); color: #6A1B9A; border-radius: 3px; font-size: 8px;">{reading}</span>'
                        for reading in nanori_readings
                    ]
                )
                reading_lines.append(
                    f'<div style="margin-bottom: 6px;"><span style="font-size: 12px; font-weight: bold; color: #9B59B6;">Nanori:</span> {tags}</div>'
                )
...
def process_vocab_meanings(
    ...
    for level, items in data.items():
        ...
        for item in items:
            front = f'<div style="font-size: 10px; color: #666;">Vocab - Level {level_num}</div><div style="font-size: 24px; color: #228B22; text-align: center; margin: 15px 0;">{item["character"]}</div><div style="font-size: 10px; color: #4A90E2;">Meaning</div>'
